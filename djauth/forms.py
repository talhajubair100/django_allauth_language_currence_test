...
from acc.models import CompanyUser, EmployeUser
...
class CustomeSignupForm(SignupForm):
    ...
    def save(self, request):
        ...
        if is_company == True:
            ...
            company_user = CompanyUser.objects.get_or_create(user=user)
        else:
            print('it is not a company')
            group = Group.objects.get(name="employe")
            user.groups.add(group)
            employe_user = EmployeUser.objects.get_or_create(user=user)
